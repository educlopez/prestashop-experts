---
source_url: https://devdocs.prestashop-project.org/9/development/architecture/introduction/
ps_version: [8, 9]
ingested: 2026-04-18
section: Version Migration > Architecture introduction
topic: migration
tags: [prestashop, architecture, symfony, legacy, modern]
---

[Improve this page](https://github.com/PrestaShop/docs/edit/9.x/development/architecture/introduction.md "Improve this page") [How](/9/contribute/documentation/how)
...
Basic overview of PrestaShop 9's architecture
...
Like most traditional web applications, PrestaShop is heavily **Database** -driven. This means that all data is stored there. Regardless of whether it’s used in the BO or FO, the database is the [Single Source of Truth](https://en.wikipedia.org/wiki/Single_source_of_truth).
...
- **BO API** – used to serve information to [VueJS](https://vuejs.org/) -based pages (currently Translations and Stock management),
...
The **Legacy subsystem** contains the historical, non-namespaced code inherited from previous versions. It’s [progressively being replaced since 1.6.1](https://build.prestashop-project.org/news/new-architecture-1-6-1-0/) by the **Core subsystem**, which uses namespaces and is based on [SOLID principles](https://en.wikipedia.org/wiki/SOLID).

The **Adapter subsystem** acts as a bridge to legacy classes, which are often static, in order to [allow them to be injected](https://en.wikipedia.org/wiki/Dependency_inversion_principle) in Core classes.
...
Notice the dotted yellow zone labeled *temporary code*. It means that code within that zone will sooner or later be moved to the Core or PrestaShop Bundle stack. Once the zone it’s empty, it will be deleted. Of course, such a change won’t be done in a minor version, so you can expect these four stacks to be present for the whole lifetime of PrestaShop 9.
...
Whenever a Core (or PrestaShopBundle) class needs something provided by a Legacy class, instead of using the Legacy class directly, it delegates that task to an Adapter, which itself uses the Legacy class (see [Adapter pattern](https://sourcemaking.com/design_patterns/adapter)).

Here’s where it gets interesting. Generally, these Adapters implement an interface declared in Core (even though it hasn’t always been the case, new classes do). Making consumers of that Adapter depend on the *interface* instead of the Adapter class itself (see [Dependency Inversion principle](https://en.wikipedia.org/wiki/Dependency_inversion_principle)) will allow to reimplement Adapter classes in Core progressively, without having to change the existing code that depends on them.

**Why not use the Legacy class directly?**

Most legacy classes are static, and since by definition they cannot be injected, it would result in coupled, untestable code. In addition, the ones that are not static generally still have too many responsibilities (see [Single responsibility principle](https://en.wikipedia.org/wiki/Single_responsibility_principle)) and/or too many public methods or properties (see [Open/closed principle](https://en.wikipedia.org/wiki/Open/closed_principle)), so they cannot be made to implement a proper interface.

### Controllers

PrestaShop is based on the [Model-View-Controller (MVC) pattern](https://en.wikipedia.org/wiki/Model%E2%80%93view%E2%80%93controller), where Controllers are in charge of handling requests and returning responses, ideally delegating the hard work on dedicated services.
...
<table><thead><tr><th align="center">Family</th><th align="center">Subsystem</th><th align="center">Type</th><th>Base controller class</th></tr></thead><tbody><tr><td align="center" rowspan="2">FO</td><td align="center" rowspan="2">Legacy</td><td align="center">Native</td><td><code>FrontController</code></td></tr><tr><td align="center">Module</td><td><code>ModuleFrontController</code> (based on <code>FrontController</code>)</td></tr><tr><td align="center" rowspan="5">BO</td><td align="center" rowspan="2">Legacy</td><td align="center">Native</td><td><code>AdminController</code></td></tr><tr><td align="center">Module</td><td><code>ModuleAdminController</code> (based on <code>AdminController</code>)</td></tr><tr><td align="center" rowspan="3">Symfony</td><td align="center">Native</td><td><code>FrameworkBundleAdminController</code></td></tr><tr><td align="center">Native (BO API)</td><td><code>ApiController</code></td></tr><tr><td align="center">Module</td><td><code>FrameworkBundleAdminController</code></td></tr></tbody></table>
...
Since FO themes work on top of legacy controllers, they are based on the [Smarty templating engine](https://www.smarty.net/). They all integrate a shared core javascript library which is called `core.js`, which has jQuery 3 bundled in.

*Classic* is based on [Bootstrap](https://getbootstrap.com/) 4 alpha 5. A great number of themes are based on it using the [Child theme feature](https://devdocs.prestashop-project.org/9/themes/reference/template-inheritance/parent-child-feature/).

Unfortunately, *Classic* [cannot be updated to recent Bootstrap versions](https://github.com/PrestaShop/PrestaShop/issues/14533#issuecomment-511464778) due to big breaking changes introduced by Bootstrap since the original release of PrestaShop 1.7.0.

Additionally, FO themes can redefine the layout of modules by [overriding their templates](https://devdocs.prestashop-project.org/9/themes/reference/overriding-modules/).
...
So why are there two? Legacy controllers are based on Smarty, like FO controller. Symfony controllers, conversely, are based on the [Twig templating engine](https://twig.symfony.com/). As a result, there’s a theme for each one: legacy controllers use the **default** theme, while Symfony ones use the **new theme**. As controllers are progressively being migrated to Symfony, templates are moved from the **default** to the **new theme** and converted from Smarty to Twig.

In addition, the **default** theme is based on Bootstrap 3, while the **new theme** is based on [PrestaShop’s UI kit](https://build.prestashop-project.org/news/PrestaShop-UI-Kit/) (available on [GitHub](https://github.com/PrestaShop/prestashop-ui-kit/)), which itself is based on Bootstrap 4. They both integrate jQuery 3.

##### Mixed Smarty and Twig

Here’s what was said when [announcing the architecture of 1.7](http://build.prestashop-project.org/news/prestashop-1-7-and-symfony/):
...
Finally, there’s [Vue pages](https://build.prestashop-project.org/news/introducing-vuejs-symfony-api/). Vue pages are hybrid: half-Symfony, half-API based BO pages. In those pages, the page’s skeleton is first rendered by a Symfony controller (therefore, based on the **new theme**), and then a VueJS application takes over in the browser and draws its content based on data sent by the BO API.

As stated before, currently only the Stock management and Translation pages are built on this technology. Even though we think that this is [the way of the future](https://build.prestashop-project.org/news/prestashop-in-2019-and-beyond-part-3-the-future-architecture/), we find that going down this path in minor version releases would produce too many major extensibility and backwards incompatibility issues. Therefore, there will probably be no new Vue/BO API pages in PrestaShop 9.
...
![](../img/architecture-comprehensive-overview-current.png)

Comprehensive overview

## Related in KB

- [[architecture-modern|Modern architecture (Symfony)]]
- [[architecture-legacy|Legacy architecture]]
- [[migration-guide-bo|Back Office migration guide]]
